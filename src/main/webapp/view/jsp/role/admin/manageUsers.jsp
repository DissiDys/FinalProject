...
<body>
    ...
    <table>
        ...
        </tr><fmt:message key="manage_users" bundle="${bundle}"/>
        <c:forEach items="${pageContext.request.getAttribute('usersList')}" var="user" varStatus="status">
            <c:if test="${user.login!='Admin'}">
                <tr>
                    ...
                    <td><a href="${pageContext.request.contextPath}/app/deleteUser?id=${user.id}"><fmt:message key="delete" bundle="${bundle}"/></a><hr/></td>
                </tr>
            </c:if>
        </c:forEach>
    </table>
